...
Все изменения находятся в ветке:

```
arena/019fb764-water-filter
```

Не в `main`. Если открыть репозиторий обычной ссылкой, GitHub покажет
ветку `main`, и новых файлов там не будет — это нормально.
...
Самый простой способ — открыть прямую ссылку сразу в режиме
редактирования нужной ветки:

```
https://github.com/Help-yourself-dvp/water-filter/edit/arena/019fb764-water-filter/.github/workflows/build.yml
```

Скопируйте эту ссылку в адресную строку браузера и нажмите Enter.
Вы сразу попадёте в редактор файла. Переходите к правке 1.

---

### Если хотите пройти путь вручную
...
3. Выберите ветку:
   ...
   arena/019fb764-water-filter
   ```
   **Это обязательный шаг.** По умолчанию GitHub показывает ветку `main`,
   а все новые файлы находятся в ветке `arena/019fb764-water-filter`.
...
Это **строки 50 и 51**. Для поиска нажмите `Ctrl + F`
...
**Что удалить: строки с 63 по 113 включительно.**

Строка 63 — это начало блока (`Ctrl + F`, введите `Patch MainActivity`):

```
      - name: Patch MainActivity for exact alarms and battery optimization
```

Строка 113 — это конец блока, она выглядит так:

```
          EOF
```

Проверьте себя: сразу после строки 113 идёт пустая строка 114,
а затем строка 115:
...
Строку 115 и всё, что ниже, **удалять не нужно**.

Итого удаляется 51 строка — весь блок от `- name: Patch MainActivity ...`
до `          EOF` включительно.
...
Это **строки 240–249**, в самом конце файла
(`Ctrl + F`, введите `Rename APK`).

Обратите внимание: после правки 2 файл стал длиннее,
поэтому номера строк сместятся вниз. Ищите по тексту, а не по номеру.
...
1. Перейдите на вкладку **Actions** вверху страницы репозитория.
   Прямая ссылка:
   `https://github.com/Help-yourself-dvp/water-filter/actions`
